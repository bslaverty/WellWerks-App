...
class _ActiveShiftScreenState extends State<ActiveShiftScreen> {
          ...
          child: Column(
            ...
            children: [
              ...
              _ActionButton(
                  ...
                  label: 'New Round',
                  onTap: () => _openFromSheet(const PressureEntryScreen())),
              _ActionButton(
                  icon: Icons.inventory,
                  label: 'Inventory / Tank Check',
                  onTap: () => _openFromSheet(
                      const ReportTemplateScreen(initialSection: 'inventory'))),
              _ActionButton(
                  icon: Icons.precision_manufacturing,
                  label: 'Equipment Event',
                  onTap: () => _openFromSheet(const EquipmentScreen())),
              _ActionButton(
                  icon: Icons.table_chart,
                  label: 'Production Report',
                  onTap: () => _openFromSheet(const ShiftReportScreen())),
            ],
          ),
        ),
      ),
  ...
  List<_AttentionItem> _attentionItems(RoundReading? latest) {
    ...
    if (latest == null) {
      items.add(const _AttentionItem(
          'No round saved',
          'Tap Quick Round to enter the first readings.',
    ...
    if (latest.casingPressure.trim().isEmpty) {
      items.add(const _AttentionItem(
          'CSG missing',
          'Casing pressure is needed for most reports.',
          Icons.warning,
          Color(0xFFFFD45D)));
    }
    if (latest.oilRate.trim().isEmpty ||
        latest.waterRate.trim().isEmpty ||
        latest.gasRate.trim().isEmpty) {
      items.add(const _AttentionItem(
          'Production incomplete',
          'Oil, water, and gas are not all entered.',
          Icons.speed,
          Color(0xFFFFD45D)));
    }
    if (latest.separatorPressure.trim().isEmpty) {
      items.add(const _AttentionItem('SP missing',
          'Separator pressure is blank.', Icons.speed, Color(0xFFFFD45D)));
    }
    if (items.isEmpty) {
      items.add(const _AttentionItem(
          'Shift looks current',
          'Production and core pressures are entered.',
          Icons.check_circle,
          Color(0xFF55D980)));
    }
    return items;
  ...
  String _chokeValue(RoundReading? reading) {
    ...
    if (style.isEmpty || lower.contains(' adj') || lower.contains(' pos')) {
      return saved;
    }
    return '$saved $style';
  }
  ...
  Widget build(BuildContext context) {
    ...
    final lastUpdated = latest == null
        ? 'No rounds saved yet'
        : DateFormat('h:mm a').format(latest.timestamp);
      ...
      body: ListView(
        ...
        children: [
                ...
                children: [
                  ...
                  Row(children: [
                    Expanded(
                        child: _StatusTile(
                            label: 'Oil',
                            value: _value(latest?.oilRate ?? '', '—'),
                            unit: 'BBL/hr')),
                    Expanded(
                        child: _StatusTile(
                            label: 'Water',
                            value: _value(latest?.waterRate ?? '', '—'),
                            unit: 'BBL/hr')),
                  ]),
                  const SizedBox(height: 12),
                  Row(children: [
                    Expanded(
                        child: _StatusTile(
                            label: 'Gas',
                            value: _value(latest?.gasRate ?? '', '—'),
                            unit: 'MCFD')),
                    Expanded(
                        child: _StatusTile(
                            label: 'CSG',
                            value: _value(latest?.casingPressure ?? '', '—'),
                            unit: 'psi')),
                  ]),
                  const SizedBox(height: 12),
          ...
          _InfoCard(
            ...
            children: _attentionItems(latest)
                .map((item) => _AttentionRow(item))
                .toList(),
          ),
          const SizedBox(height: 16),
          _InfoCard(
            ...
            children: [
              _Line(
                  label: 'TBG',
                  value: _value(latest?.tubingPressure ?? '', '—')),
              _Line(
                  label: 'CSG',
                  value: _value(latest?.casingPressure ?? '', '—')),
              ...
              _Line(
                  label: 'SP',
                  value: _value(latest?.separatorPressure ?? '', '—')),
              _Line(
                  label: 'Diff',
                  value: _value(latest?.differentialPressure ?? '', '—')),
            ],
          ),
          const SizedBox(height: 16),
          _InfoCard(
            ...
            children: [
              _Line(
                  label: 'Gas TMP', value: _value(latest?.gasTemp ?? '', '—')),
              _Line(
                  label: 'WH TMP',
                  value: _value(latest?.wellheadTemp ?? '', '—')),
              _Line(
                  label: 'H2O TMP',
                  value: _value(latest?.waterTemp ?? '', '—')),
              _Line(
                  label: 'ECD TMP', value: _value(latest?.ecdTemp ?? '', '—')),
              _Line(label: 'Prop', value: _value(latest?.propRate ?? '', '—')),
              _Line(
                  label: 'Biocide',
                  value: _value(latest?.biocideRate ?? '', '—')),
            ],
          ),
          const SizedBox(height: 16),
          Text('Round History',
          ...
          ..._readings.take(10).map((r) => Card(
                ...
                child: ListTile(
                  title: Text(r.roundLabel.isEmpty
                      ...
                      : r.roundLabel),
                  subtitle: Text(
                      'Oil ${_value(r.oilRate, '—')} • Water ${_value(r.waterRate, '—')} • Gas ${_value(r.gasRate, '—')} • CSG ${_value(r.casingPressure, '—')}'),
                ),
              )),
