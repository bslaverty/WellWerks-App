...
class _JsaScreenState extends State<JsaScreen> {
  ...
  final Map<String, Map<String, List<String>>> _taskLibrary = const {
    ...
    'Change Choke': {
      'steps': ['Notify crew', 'Verify choke position', 'Make controlled adjustment', 'Monitor pressures'],
      'hazards': ['Pressure change', 'Line of fire', 'Pinch points', 'Unexpected flow change'],
      'recommendations': ['Communicate before adjustment', 'Stand clear of pressure points', 'Use proper tools', 'Record change and response'],
    },
    'Pressure Test': {
      'steps': ['Inspect connections', 'Clear test area', 'Bring pressure up slowly', 'Monitor for leaks', 'Bleed down safely'],
      'hazards': ['High pressure', 'Failed iron', 'Flying debris', 'Stored energy'],
      'recommendations': ['Use rated iron', 'Keep non-essential personnel clear', 'Never stand in line of fire', 'Confirm bleed down before work'],
    },
  };
  ...
  void _toggleTask(String task, bool selected) {
    setState(() {
      if (selected) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Select at least one JSA step.')));
      }
    });
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const Text('Select everything that applies to this job.', style: TextStyle(color: Colors.white70)),
          const SizedBox(height: 12),
          Wrap(
            spacing: 10,
            runSpacing: 10,
